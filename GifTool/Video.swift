//
//  Video.swift
//  GifTool
//
//  Created by Nah on 12/15/17.
//  Copyright © 2017 Nah. All rights reserved.
//

import Foundation
import Unbox

struct Videos: Unboxable {
    let videos: [Video]
    
    init(unboxer u: Unboxer) throws {
        self.videos = try u.unbox(key: "greetings")
    }
}

struct Video {
    
    let id: String
    let title: String
    
    let videoId: String
    let videoTitle: String
    let youtubeId: String
    
    let artist: String
    let category: Int
    let videoPath: String
    
    let isComplied: Bool
    let isCompleted: Bool
    
    let commentSpeed: Int
    let start: TimeInterval
    let end: TimeInterval
}

extension Video: Unboxable {
    
    init(unboxer u: Unboxer) throws {
        self.id = try u.unbox(key: "Id")
        self.title = try u.unbox(key: "Title")
        
        self.videoId = try u.unbox(key: "VideoId")
        self.videoTitle = try u.unbox(key: "VideoTitle")
        self.videoPath = try u.unbox(key: "Path")
        
        self.youtubeId = try u.unbox(key: "YoutubeId")
        self.artist = try u.unbox(key: "Artist")
        self.category = try u.unbox(key: "Category")
        
        self.isComplied = try u.unbox(key: "isCompiled")
        self.isCompleted = try u.unbox(key: "isCompleted")
        
        self.commentSpeed = try u.unbox(key: "CommentSpeed")
        self.start = try u.unbox(key: "StartSec")
        self.end = try u.unbox(key: "EndSec")
    }
}


/*
 Artist = "Katy Perry";
 Category = 3;
 CommentSpeed = 6;
 EndSec = 138;
 Filename = "video___iGk5fR-t5AU___bbmf9jr1dtrhf5eewpqfkobwa7";
 Id = 490290943194080;
 Path = "http://berserker6.vibbidi-vid.com/vibbidi-us/videos/video___iGk5fR-t5AU___bbmf9jr1dtrhf5eewpqfkobwa7.mp4";
 StartSec = 130;
 Title = "Stealing Memes To Stay Relevant";
 VideoId = 752172503583574;
 VideoTitle = "Swish Swish";
 YoutubeId = "iGk5fR-t5AU";
 isCompiled = 0;
 isCompleted = 1;
 */
